...
export default ({data, onPressDelete}) => {
    ...
    return (
        <Box onPress={handleMostrar}
            ...
            style={{marginLeft: 20, marginRight: 20, backgroundColor: '#F8F8F9', borderLeftColor: '#CDCDD2', borderRightColor: '#CDCDD2', borderTopColor: '#CDCDD2', borderLeftWidth: 1, borderRightWidth: 1, borderTopWidth: 1}}
        >
            <HeaderArea>
                <InfoArea>
                    <InfoAreaBox style={{flex:3, flexDirection: 'row'}}>
                        <Image source={{uri: data.img}} style={{width:50, height:50, borderColor: '#CDCDD2', borderWidth: 1}} resizeMode="cover" />
                        <Date style={{flex:1, marginLeft: 10}}>{data.nome}</Date>
                    </InfoAreaBox>
                    <InfoAreaBox style={{width:100, alignItems: 'flex-end',}}>
                    <View style={{position:'absolute', top:25, right:30, justifyContent:'center', alignItems: 'center'}}>
                            {!mostrar && 
            ...
            {mostrar && 
                <MostrarBox>
                    ...
                    <MostrarBoxItem>
                    <MostrarBoxItemText>Código do produto:</MostrarBoxItemText>
                    <MostrarBoxItemText>Preço do produto:</MostrarBoxItemText>
                    <MostrarBoxItemText>Quantidade selecionada:</MostrarBoxItemText>
                    </MostrarBoxItem>

                    <MostrarBoxItem style={{alignItems: 'flex-end'}}>
                    <MostrarBoxItemText>{data.codigo_produto}</MostrarBoxItemText>
                    <MostrarBoxItemText>R$ {parseFloat(data.valor_unitario).toFixed(2).toString().replace('.', ',')}</MostrarBoxItemText>
                        
                        <MostrarBoxItemText>{data.quantidade}</MostrarBoxItemText>
                       
                       
                        
                        <ButtonArea style={{width: 100, backgroundColor: '#bd2130'}} onPress={() => {onPressDelete(data.codigo_produto)}}>
